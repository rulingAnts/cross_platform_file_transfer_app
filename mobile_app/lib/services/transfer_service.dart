/**
 * Rapid Transfer - Transfer Service (TLS/TCP)
...
-----END PRIVATE KEY-----''';
  ...
  Future<void> _processIncomingBuffer(
    SecureSocket socket,
    List<int> buffer,
  ) async {
  ...
  Future<void> _handleIncomingTransferRequest(
    ...
    // final checksum = message['checksum']; // Reserved for future checksum verification
    ...
    try {
      ...
      if (transfer.isDirectory) {
        final extractDir = Directory(
          finalPath.replaceAll('.tar.gz', '').replaceAll('.zip', ''),
        );
      ...
      await Directory(
        transfer.filePath,
      ).delete(recursive: true).catchError((_) => Directory(transfer.filePath));
  ...
  Future<void> _addToHistory(
    ...
    try {
      ...
      final duration = completedAt.difference(transfer.createdAt);
  ...
  Future<void> _startTransfer(FileTransfer transfer) async {
    ...
    try {
      ...
      // Get temp directory
      final tempDir = await getTemporaryDirectory();
      final transferTempDir = Directory(
        '${tempDir.path}/rapidtransfer/send/${transfer.id}',
      );
      ...
      // Cleanup
      await transferTempDir
          .delete(recursive: true)
          .catchError((_) => transferTempDir);
      socket.close();
    ...
    Timer(const Duration(seconds: 30), () {
      if (!completer.isCompleted) {
        completer.completeError(
          TimeoutException('Transfer acceptance timeout'),
        );
      }
    });
  ...
  Future<void> _waitForChunkAck(SecureSocket socket, int chunkIndex) async {
    ...
    Timer(const Duration(seconds: 10), () {
      if (!completer.isCompleted) {
        completer.completeError(
          TimeoutException('Chunk acknowledgment timeout'),
        );
      }
    });
  ...
  Future<void> cancelTransfer(String transferId) async {
    ...
    if (transfer != null) {
      ...
      final tempDir = await getTemporaryDirectory();
      final transferTempDir = Directory(
        '${tempDir.path}/rapidtransfer/send/$transferId',
      );
      await transferTempDir
          .delete(recursive: true)
          .catchError((_) => transferTempDir);
